...
startingPlayer = input('Secect Starting player either X == "AI" or O == "You" = ')
currentPlayer = startingPlayer

def getBoardCopy(board):  ## clone main board for simulation
    boardCopy = []
...
def hasMovesLeft(board):   ## check any move left 
...
def hasdrawn(currentBoard):
    cx = 0
    co = 0
    for y in range(boardSize):
        for x in range(boardSize):
          cxa = currentBoard[y][x].count('X')
          coa = currentBoard[y][x].count('O')
          cx = cxa + cx 
          co = coa + co 
    if (startingPlayer == 'X' and cx == 5) or (startingPlayer == 'O' and co == 5):
          return True
...
def getBestNextMove(currentBoard, currentPlayer):  ## The simulations
    evaluations = {}
